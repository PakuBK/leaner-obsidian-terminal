...
function getShellArgs(shellPath: string, startupCommand?: string): string[] {
    if (Platform.isWin) {
...
export class PtyManager {
    ...
    spawn(
        ...
        const args = getShellArgs(shell, startupCommand);

        // No shell integration injection: behave like a plain terminal wrapper.
        // Hint for prompt renderers (Oh My Posh, etc.) to use truecolor sequences.
        ...
        this.ptyProcess = this.nodePty.spawn(shell, args, {
            ...
            cwd,
            env: ptyEnv,
            // Default to WinPTY for compatibility; allow opt-in ConPTY.
            useConpty: Platform.isWin ? Boolean(useConpty) : undefined,
        });
    }
